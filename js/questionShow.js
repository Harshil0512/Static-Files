    ...
    success:(result)=>{
        ...
        for (let i in jd) {
            for (let j = 0; j < jd[i].length; j++) {
              ...
              var qc=0
              temp1+=``;
                temp1 += `<div class="accordion__item accordion__item--active"><p class="countno">${qcount}</p>`;
      ...
      success:function (result) {
        ...
        for (let i in jd) {
          for (let j = 0; j < jd[i].length; j++) {
            ...
            const match2 = jd[i][j]['responses'].match(new RegExp(ip, "gi"))
            isMatched = match.some(key => key);
            if (isMatched || match2!=null) {
              for (n in jd[i][j]['patterns']) {
                temp1 += `<button class="accordion__btn">
                <span class="accordion__caption"><i class="far fa-lightbulb"></i>${jd[i][j]['patterns'][n]}</span>
              </button>`;
